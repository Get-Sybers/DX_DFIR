...
- **The analysis backend holds evidence.** The Elastic-native stack
  ...
  the gitignored `docker/elastic/.env`. Every published port binds `127.0.0.1`;
  that binding is a real control, the rest is best effort.
...
- The backend holds ingested evidence on localhost (named Docker volumes for the
  Elastic stack); Filebeat reads the processed tree the stack mounts read-only,
  and the ingested copies outlive the run — purge them with the case.
