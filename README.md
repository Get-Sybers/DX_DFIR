...
Kibana is at `http://127.0.0.1:5601`. Filebeat ships the per-type processed tree
under `data_store/processed/` (`<type>/**/*.json[l]`) into
...
Python package** — writing the processed tree the CAR lane builds from and the
Elastic stack ships. Each source runs as `dxdfir process <source>` (driving the
