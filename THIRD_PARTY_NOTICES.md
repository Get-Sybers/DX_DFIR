...
| [EvtxECmd](https://github.com/EricZimmerman/evtx) | `get_sybers_dxdfir.evtx` runs `EvtxECmd.dll` in a .NET container — either the bundled `get-sybers/evtxecmd` image (`third_party/EZTools-Docker/evtxecmd`, fetches the release at build time) or an operator-supplied release | **MIT** | None — no commercial-use restriction |
...
(e.g. `third_party/EZTools-Docker/evtxecmd`), or supplied by the operator.
