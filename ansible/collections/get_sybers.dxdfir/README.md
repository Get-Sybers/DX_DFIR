...
| Ansible-only container interaction | **Strict rule:** containers are built, run and removed ONLY by ansible — the lanes (`dxdfir_lane`), the image lifecycle (`dxdfir_images`), the stack (`dxdfir_stack`), the wipe (`dxdfir_cleanup`) and the molecule harness (`playbooks/dxdfir-molecule.yml`) are committed playbooks/roles; no script, python or Go code issues a container lifecycle command. Read-only daemon **observation** is the one sanctioned exception: the CLI health probe's `docker info` and the TUI's `ps`/`stats` monitor observe, never touch. |
