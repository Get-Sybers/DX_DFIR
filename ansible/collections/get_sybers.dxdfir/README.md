...
Stack role: **`dxdfir_stack`** brings the Elastic-native analysis backend
(`docker/elastic/`) up/down via docker compose (deploy/destroy/start/stop/status).
Its Filebeat reads the processed output tree (`data_store/processed/<type>/…`)
directly — there is no separate delivery step.
...
```bash
dxdfir process zeek        # = the dxdfir_zeek role, preflight → process → verify
...
| Variables, no hardcoding | Everything flows through `defaults/main.yml` with the `dxdfir_<role>_` prefix; inputs validated with `assert` at play start; each role's output path is a single resolved variable (`dxdfir_<role>_out_dir`), not duplicated task files. |
...
| Cross-platform conditionals | **Principle implemented, mechanics inapplicable:** the practice's point — one adaptable unit instead of near-identical copies — is exactly the shared `process.yml` skeleton every lane role reuses (preflight → process → verify → gate), parameterised by `dxdfir_<role>_*`. OS-family `when` ladders have no surface: targets are tool containers on a Linux analysis host, single-platform by design. |
