...
|---|---|---|
| `dxdfir_anamnesis_memory_dir` | `<repo>/data_store/raw/memory` | Memory-image tree to process (recursed); mounted read-only at `/input` inside the container. |
| `dxdfir_anamnesis_out_dir` | `<repo>/data_store/processed/anamnesis[/<collection>]` | Output base (override to redirect). |
| `dxdfir_anamnesis_symbols_dir` | `<repo>/data_store/dependencies/memprocfs-symbols` | Persistent MemProcFS symbol cache (symsrv layout); mounted read-write at `/opt/anamnesis/lib/Symbols`. |
| `dxdfir_anamnesis_image` | `get-sybers/anamnesis:latest` | The hardened, env-driven anamnesis (MemProcFS) image the lane docker-runs (built by `playbooks/dxdfir-build-images.yml`). |
| `dxdfir_anamnesis_force` | `false` | Rerun plugins that already have valid output. |

## Symbols (the persistent cache mount)
The PDB symbols the Windows fields need (`command_line`, `sid`, `user`, …)
live in `dxdfir_anamnesis_symbols_dir`, bind-mounted **read-write** at
