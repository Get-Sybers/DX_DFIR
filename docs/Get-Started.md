...
The processors write the tree the CAR lane builds from and the Elastic stack
ships. `man dxdfir` for the manual.
...
- Filebeat tails the tree mounted at `ELASTIC_INGEST_DIR` (`<type>/**/*.json[l]`)
  ...
  point `ELASTIC_INGEST_DIR` at the per-type processed tree under
  `data_store/processed/` (`zeek/`, `plaso/`, …).
