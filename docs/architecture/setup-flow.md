...
| 8 | **Ansible collections** | `ansible-galaxy install` the collection's pinned `requirements.yml` into `/opt/dxdfir/collections`. The GoDFIR-toolz **build galaxy** (`get_sybers.godfir_toolz`) installs NOTHING: the committed collection symlink resolves it in place from the submodule at the gitlink pin (no copy, no tarball); only a checkout without the submodule imports it from the `.gitmodules` source at the gitlink revision. |
