...
| 8 | **Ansible collections** | `ansible-galaxy install` the collection's pinned `requirements.yml` into `/opt/dxdfir/collections`. The GoDFIR-toolz **build galaxy** installs NOTHING: its roles resolve in place from the submodule (`docker/GoDFIR-toolz/roles` on the repo-root `roles_path`) at the gitlink pin — no copy, no tarball; only a checkout without the submodule materialises the tree from the `.gitmodules` source at the gitlink revision. |
