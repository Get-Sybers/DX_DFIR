...
| 8 | **Ansible collections** | `ansible-galaxy install` the collection's pinned `requirements.yml` into `/opt/dxdfir/collections`, then import the GoDFIR-toolz **build galaxy** (`get_sybers.godfir_toolz`) from the submodule — or, when its content is absent, from the source the repo root declares (`.gitmodules` URL at the gitlink revision). |
