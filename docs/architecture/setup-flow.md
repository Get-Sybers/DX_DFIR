...
| 6 | **Python + Ansible** | Create `/opt/dxdfir/venv`, `pip install --editable python/` against `python/constraints.txt`; the venv's `ansible*` reach PATH through the profile.d drop-in of step 7 — no symlink shims. |
