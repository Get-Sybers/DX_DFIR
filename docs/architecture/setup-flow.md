...
| 6 | **Ansible collections** | `ansible-galaxy install` the collection's pinned `requirements.yml` into the checkout's own `.ansible/collections` (gitignored, as the invoking user — the first entry of `ansible.cfg`'s `collections_path`), with ansible's state home (`ANSIBLE_HOME`, `ansible.cfg`'s `home`) in the checkout's `.ansible/` too, so the galaxy download staging and cache never touch `~/.ansible`; the retired `/opt/dxdfir` prefix is removed once nothing lives there. The GoDFIR-toolz **build galaxy** installs NOTHING on the primary path: its roles resolve in place from the submodule (`docker/GoDFIR-toolz/roles` on the repo-root `roles_path`) at the gitlink pin; only a checkout without the submodule has it imported by `ansible-galaxy` from the `.gitmodules` source at the gitlink revision, into the shared path `roles_path` also covers as its degraded-only last entry. |
