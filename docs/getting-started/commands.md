...
| Command | What it does |
|---|---|
| `dxdfir stack deploy [--no-build]` | Build (if needed), start, and verify the Elastic analysis stack. |
| `dxdfir stack start` / `stop` / `status` | Start / stop / show the stack's containers. |
