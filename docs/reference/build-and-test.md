...
Python processors, the `dxdfir_images` role, `sources.yml`, `.gitmodules`,
`docker/GoDFIR-toolz`, `images.yml`/`sources.yml`, or the smoke test). It checks out submodules recursively and builds the
