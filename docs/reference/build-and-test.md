...
`tests/run-checks.sh` (also `dxdfir validate`) is static and internal-consistency only —
it does **not** run the pipeline. Wired into CI via `.github/workflows/checks.yml`.

```bash
./tests/run-checks.sh          # or: dxdfir validate   ·   -v for verbose
```

Groups it runs:

- **Shell** — `bash -n` + `shellcheck -S error` over `scripts/`, `dev-scripts/`, `tests/`.
...
`.github/workflows/smoke.yml` → `tests/smoke-test.sh` runs the pipeline end to end for
...
./tests/run-checks.sh                                   # everything
```
