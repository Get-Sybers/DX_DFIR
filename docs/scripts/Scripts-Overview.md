...
| `save-docker-images.sh` | THE image engine of the offline lifecycle: saves the built hardened `get-sybers/*` set (from `images.yml`) **plus the Elastic stack's `docker.elastic.co/*` images** (`docker/elastic`, at `ELASTIC_VERSION`) as tarballs; `--build` first runs `dxdfir build-docker`; `--load` / `--verify` restore them and assert the hardened inventory. Both offline entry points call it. |
| `package-offline.sh` | The connected-side entry point: build ONE portable air-gap bundle — images via `save-docker-images.sh` (the `get-sybers/*` tool set incl. the baked-in Byakugan engine, plus the Elastic stack images, so the backend deploys offline with zero pulls), the `get_sybers_dxdfir` processor package + deps as wheels, the Go front-end's vendored modules (`go-vendor.tar`), pinned collections, the repo, and `data_store/dependencies/` (YARA/Suricata/Hayabusa rulesets + binary, Volatility symbols) under a `MANIFEST.sha256`. The saved images are native, so the bundle is arch-specific. Run `setup-environment.sh` first, then this with `--build`. |
| `setup-offline.sh` | The air-gapped-side entry point — it replaces `setup-environment.sh` there (whose docker/apt/go/pip/galaxy steps all need the network): manifest-verify everything, unpack the repo + detection dependencies, load every image via `save-docker-images.sh --load` (tool set + Elastic stack), install the package/collections offline, build the Go `dxdfir` front-end from the vendored modules, finish with `dxdfir verify-images` (or the same audit play via `ansible-playbook` when no Go toolchain is present). |
