...
| [EZ-Tools](https://ericzimmerman.github.io/) (Zimmerman) artefacts | ✅ the godfir-toolz lane (`dxdfir process godfir-toolz`) | json / csv | ✅ `godfir-toolz/` | ✅ (`registry`/`flow`/`process`) |
...
- ~~**Raw EVTX processing is built but unverified.**~~ **Resolved.** The `dxdfir_evtx`
  ...
  `get-sybers/goevtx` image), 55,638 EvtxECmd-shape rows, feeding the
...
- ⬜ The same for a small public disk image (the plaso / godfir-toolz lanes).
